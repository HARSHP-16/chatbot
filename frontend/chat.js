document.addEventListener('DOMContentLoaded', () => {
    ...
    // 0. Dynamic Initialization for Stats
    const refreshBoardStats = () => {
        try {
            const eventsStore = JSON.parse(localStorage.getItem('campusEventsStore') || '[]');
            const notifStore = JSON.parse(localStorage.getItem('campusNotificationsStore') || '[]');

            const eventsCounterEl = document.getElementById('eventsCounter');
            const notifBadgeEl = document.getElementById('notifBadgeCount');
            const placementCounterEl = document.getElementById('placementCounter');
            const hostelCounterEl = document.getElementById('hostelCounter');

            if (eventsCounterEl) {
                eventsCounterEl.setAttribute('data-target', String(eventsStore.length));
                eventsCounterEl.textContent = String(eventsStore.length);
            }

            if (notifBadgeEl) {
                notifBadgeEl.textContent = String(notifStore.length);
            }

            // Derive lightweight board numbers from active data to keep student board fresh.
            if (placementCounterEl) {
                const placementRelated = eventsStore.filter((e) => {
                    const text = `${e.title || ''} ${e.description || ''}`.toLowerCase();
                    return text.includes('placement') || text.includes('drive') || text.includes('internship');
                }).length;
                placementCounterEl.setAttribute('data-target', String(placementRelated));
                placementCounterEl.textContent = String(placementRelated);
            }

            if (hostelCounterEl) {
                const hostelRelated = notifStore.filter((n) => {
                    const text = `${n.title || ''} ${n.message || ''}`.toLowerCase();
                    return text.includes('hostel') || text.includes('mess') || text.includes('room');
                }).length;
                hostelCounterEl.setAttribute('data-target', String(hostelRelated));
                hostelCounterEl.textContent = String(hostelRelated);
            }

            const timetableStore = JSON.parse(localStorage.getItem('campusTimetableStore') || '{}');
            const classesCounterEl = document.getElementById('classesCounter');
            if (classesCounterEl && Object.keys(timetableStore).length > 0) {
                let totalFilteredClasses = 0;
                Object.values(timetableStore).forEach(deptArray => {
                    deptArray.forEach(day => {
                        const count = day.slots.filter(s => s && s.toLowerCase() !== 'free' && s.trim() !== '').length;
                        totalFilteredClasses += count;
                    });
                });
                classesCounterEl.setAttribute('data-target', String(totalFilteredClasses));
                classesCounterEl.textContent = String(totalFilteredClasses);
            }
        } catch (e) {
            console.warn('Failed to refresh board stats', e);
        }
    };
    ...
    if (notificationBtn) {
        notificationBtn.addEventListener('click', () => {
            window.location.href = 'feature.html#notifications';
        });
    }
    ...
    // 4. Cross-tab Realtime Reactivity!
    window.addEventListener('storage', (e) => {
        if (e.key === 'campusEventsStore') {
            try {
                const eventsStore = JSON.parse(e.newValue || '[]');
                const eventsCounterEl = document.getElementById('eventsCounter');
                if (eventsCounterEl) {
                    eventsCounterEl.setAttribute('data-target', eventsStore.length);
                    eventsCounterEl.innerText = eventsStore.length; // Snap instantly!
                }
            } catch(e) {}
        }
    });
